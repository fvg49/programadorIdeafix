...
wcmd(contden, %I% %G% )
{
	...
	if ((cmd = DoForm(fm0, before, after)) != FM_EXIT) {
        ...
        if (v_destino==DESTINO_TEST || v_destino==DESTINO_PASE) {
			...
			// Se controla que no haya procesos que graben el parte abiertos antes de empezar el cierre.
			// cuales son estos procesos se configuran en comgral depano PARNOV_GRABAN_PARTE
			// dentro de esos proceso tambien hay un control que verifica que no haya un cierre 
			// ejecutando para que sean procesos mutuamente excluyentes

			if (ProcesosBloqueanPartes()) {
				sprintf(v_msgerr,"%s", NULL_STR);
			//	v_inipar= LisParNov(PARNOV_EMPRESA_GENERAL, PARNOV_GRABAN_PARTE, Today(), _ORDEN_PARNOV_STR_ASC);
				v_inipar= LisParNov(PARNOV_EMPRESA_GENERAL, 55, Today(), _ORDEN_PARNOV_STR_ASC);
				for (nodo_aux=v_inipar; nodo_aux!=NULL; nodo_aux=(*nodo_aux).nsig) {
					sprintf(v_msgerr,"%s %s \n", v_msgerr, (*nodo_aux).parame);
				}
				sprintf(v_msgerr,"%s No podra ejecutarse el cierre \nhasta cerrar esos procesos", v_msgerr);

				WiDialog(WD_OK, WD_OK, "Hay procesos bloqueantes ejecutandose", "%s\n", v_msgerr);
...
/*	if (FmIFld(fm0, PERIOD)!=1){
		SetKey(operac|PERIODObyEMP, FmIFld(fm0, EMP), FmIFld(fm0, ANIO), FmIFld(fm0, COTICI), FmIFld(fm0, PERIOD)-1);
		if(GetRecord(operac|PERIODObyEMP, THIS_KEY, IO_NOT_LOCK) == ERROR) {
			WiDialog(WD_OK, WD_OK, "Error en el Periodo Selecionado", "El Periodo Anterior al Pedido es Inexistente, comunicarse con DTI");
			PopRecord(operac|PERIODO);
			return FALSE;
		...
		//si no se ejecuto el periodo anterior, no se puede ejecutar este
		if (IFld(operac|PERIODO_ESTADO)!=CONST_ESTADO_TERM_OK) {
			WiDialog(WD_OK, WD_OK, "Error en el Periodo Anterior al Selecionado", 
					 "El Periodo Anterior No Finalizo Exitosamente (Periodo Anterior %.3D - %.3D Estado %d)\nNo Se Procesara el Periodo Selecionado", 
					 DFld(operac|PERIODO_FECDES), DFld(operac|PERIODO_FECHAS), IFld(operac|PERIODO_ESTADO));
