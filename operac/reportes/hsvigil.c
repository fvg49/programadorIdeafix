...
void CargarVigil(short htipo, bool esexcep, int emp, char deleg[6], long cliente, int objetivo, long nroleg, 
				  DATE fecha, TIME hsent, TIME hssal, int ptoser, int puesto, int nroint, char cond[10],
...
int  g_emp;
//utilizo en la optimizacion
struct s_lisxusr_lib esta_lis;

/* Programa principal */
wcmd(hsvigil, 1.7 11/10/25 )
{
	fm_cmd cmd;
	...
	fm0 = OpenForm("hsvigil", FM_EABORT);
...
void LimpiarLista() 
{
	...
	for (evig = pvig ; evig < uvig ; evig++) {
		...
		sprintf(evig->deleg, "%s", NULL_STR);
		evig->nroleg = NULL_LONG;
		evig->cli    = NULL_LONG;
		evig->obj    = NULL_SHORT;
	    evig->fecha  = NULL_DATE;
	    evig->ptoser = NULL_SHORT;
		evig->puesto = NULL_SHORT;
		evig->nroint = NULL_SHORT;
		sprintf(evig->cond, "%s", NULL_STR);
	    evig->nrosem = NULL_SHORT;
	    evig->htipo  = NULL_SHORT;
	    evig->hsent  = NULL_TIME;
	    evig->hssal  = NULL_SHORT;
		evig->sthn = evig->sth50 = evig->sth100 = evig->sth100f = NULL_LONG;
		evig->sahn = evig->sah50 = evig->sah100 = evig->sah100f = NULL_LONG;
		evig->sehn = evig->seh50 = evig->seh100 = evig->seh100f = NULL_LONG;
		evig->sihn = evig->sih50 = evig->sih100 = evig->sih100f = NULL_LONG;
		evig->sistd = NULL_LONG;
	}
	uvig = pvig;
}
...
{
	#ifdef _NOVIA_VER_1_0
	    rp0 = OpenReport("hsvigil", RP_EABORT|RP_NOBEGIN);
	#else	
	    rp0 = OpenReport("hsvigil2", RP_EABORT|RP_NOBEGIN);
	#endif
...
void GenerarReporte()
{
	...
	bool v_setprocede[_PROCED_MAX];

	v_setprocede[_PROCED_ASIGNA]=TRUE;
	v_setprocede[_PROCED_NOASIG]=FALSE;
	v_setprocede[_PROCED_PERVIG]=TRUE;
	v_setprocede[_PROCED_GRUFIL]=FALSE;
	...
	sprintf(auxcon, "%s", NULL_STR);

	for (nodo_aux=inileg; nodo_aux!=NULL; nodo_aux=(*nodo_aux).nsig) {

		if (!FmIsNull(fm0, VIGILD) && (*nodo_aux).legxfil < FmLFld(fm0, VIGILD))
			continue;

		if (!FmIsNull(fm0, VIGILH) && (*nodo_aux).legxfil > FmLFld(fm0, VIGILH))
			break;

		SetCursorFrom(cparte, FmIFld(fm0, EMP), (*nodo_aux).legxfil, FmDFld(fm0, FECHAD), MIN_LONG, MIN_SHORT);
		SetCursorTo  (cparte, FmIFld(fm0, EMP), (*nodo_aux).legxfil, FmDFld(fm0, FECHAH), MAX_LONG, MAX_SHORT);
		while(FetchCursor(cparte) != ERROR) {

			if (!FmIsNull(fm0, DELEG) || !FmIsNull(fm0, FDELEGA) || !FmIsNull(fm0, FFILIAL))
		 		if (!ValidaFilial(LFld(PARTE_CLIENTE), IFld(PARTE_OBJETIVO), FmSFld(fm0, DELEG), FmSFld(fm0, FDELEGA), FmSFld(fm0, FFILIAL)))
					continue;

			// Muestra horas en otras filiales
			if (FmIFld(fm0, MUESHS)) {
				if (!ValidaObjetivoXusr(LFld(PARTE_CLIENTE), IFld(PARTE_OBJETIVO), IFld(PARTE_EMP))){
					v_procede=GetProcedeXFecha((*nodo_aux).nfecxleg, DFld(PARTE_DIA));
					if (v_procede==NULL_SHORT || v_procede==_PROCED_GRUFIL)
						if (!ValidaLegajoXusrXProc(LFld(PARTE_NROLEG), v_setprocede))
							continue;
				}
			}
			else { 
...
//	        fprintf(stderr, "PARTE - Cliente %ld - Objetivo %d - Delega %s - Deleg %s - Filial %s\n", LFld(PARTE_CLIENTE), IFld(PARTE_OBJETIVO), FmSFld(fm0, DELEG), FmSFld(fm0, FDELEGA), FmSFld(fm0, FFILIAL)); 
	...
	//short HorasNormalesPorSemana(short emp, long nroleg, short pais, short prov, short nrosem, DATE fechad, DATE fechah)
					...
					sprintf(auxcon, "%s%d", SFld(PARTE_CONDIC), IFld(PARTE_CODAUS));
	...
	for (nodo_aux=inileg; nodo_aux!=NULL; nodo_aux=(*nodo_aux).nsig) {
		...
		SetCursorFrom(cexc, FmIFld(fm0, EMP), (*nodo_aux).legxfil, FmDFld(fm0, FECHAD), MIN_LONG, MIN_SHORT);
		SetCursorTo  (cexc, FmIFld(fm0, EMP), (*nodo_aux).legxfil, FmDFld(fm0, FECHAH), MAX_LONG, MAX_SHORT);
		...
		while (FetchCursor(cexc) != ERROR) {
			...
			if (FmIFld(fm0, MUESHS)) {
				...
				if (!ValidaObjetivoXusr(LFld(EXCEPCION_CLIENTE), IFld(EXCEPCION_OBJETIVO), IFld(EXCEPCION_EMP))){
					v_procede=GetProcedeXFecha((*nodo_aux).nfecxleg, DFld(EXCEPCION_DIA));
					if (v_procede==NULL_SHORT || v_procede==_PROCED_GRUFIL)
						if (!ValidaLegajoXusrXProc(LFld(EXCEPCION_NROLEG), v_setprocede))
							continue;
				}
			}
			else {
	...
	if (FmIFld(fm0, FRETRO)) {
		...
		for (nodo_aux=inileg; nodo_aux!=NULL; nodo_aux=(*nodo_aux).nsig) {
			...
			SetCursorFrom(cretro, FmIFld(fm0, EMP), (*nodo_aux).legxfil, FmDFld(fm0, FECHAD), MIN_LONG, MIN_SHORT);
			SetCursorTo  (cretro, FmIFld(fm0, EMP), (*nodo_aux).legxfil, FmDFld(fm0, FECHAH), MAX_LONG, MAX_SHORT);
			...
			while(FetchCursor(cretro) != ERROR) {
				...
				// Muestra horas en otras filiales
				if (FmIFld(fm0, MUESHS)) {
					if (!ValidaObjetivoXusr(LFld(RETRO_CLIENTE), IFld(RETRO_OBJETIVO), FmIFld(fm0, EMP))){
			            v_procede=GetProcedeXFecha((*nodo_aux).nfecxleg, DFld(RETRO_DIA));
						if (v_procede==NULL_SHORT || v_procede==_PROCED_GRUFIL)
							if (!ValidaLegajoXusrXProc(LFld(RETRO_NROLEG), v_setprocede))
								continue;
					}
				}
				else {
				...
				sprintf(auxcon, "%s%d", SFld(RETRO_CONDIC), IFld(RETRO_CODAUS));
		...
		for (nodo_aux=inileg; nodo_aux!=NULL; nodo_aux=(*nodo_aux).nsig) {
			...
			SetCursorFrom(cretexc, FmIFld(fm0, EMP), (*nodo_aux).legxfil, FmDFld(fm0, FECHAD), MIN_LONG, MIN_SHORT);
			SetCursorTo  (cretexc, FmIFld(fm0, EMP), (*nodo_aux).legxfil, FmDFld(fm0, FECHAH), MAX_LONG, MAX_SHORT);
			...
			while (FetchCursor(cretexc) != ERROR) {
				...
				// Muestra horas en otras filiales
				if (FmIFld(fm0, MUESHS)) {
					if (!ValidaObjetivoXusr(LFld(RETROEXC_CLIENTE), IFld(RETROEXC_OBJETIVO), FmIFld(fm0, EMP))){
			            v_procede=GetProcedeXFecha((*nodo_aux).nfecxleg, DFld(RETROEXC_DIA));
						if (v_procede==NULL_SHORT || v_procede==_PROCED_GRUFIL)
							if (!ValidaLegajoXusrXProc(LFld(RETROEXC_NROLEG), v_setprocede))
								continue;
					}
...
{
	#ifdef _NOVIA_VER_1_0
		fprintf(fp, "%s\t%s\tVigilador\tNombre y Apellido\tCliente\tRazon Social\tObjetivo\tDescrip. Obj.\tTPuesto\tDescrip. TPuesto\tPuesto\tFecha\tRegimen\tRegimen Pto\tCond\tHr Entrada\tHr Salida\tEstandar Trabajado\t\t\t\tServicios Adicionales\t\t\tA Cargo Empresa\n", "Delegacion", "Descripcion" );
		fprintf(fp, "\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tHn\tH50\tH100\tH100F\tHn\tH50\tH100\tHn\tH50\tH100\n");
	#endif

	#ifdef _NOVIA_VER_2_0
		fprintf(fp, "%s\t%s\tVigilador\tNombre y Apellido\tCliente\tRazon Social\tObjetivo\tDescrip. Obj.\tTPuesto\tDescrip. TPuesto\tPuesto\tFecha\tCond\tEstandar Trabajado\t\t\t\tServicios Adicionales\t\t\tA Cargo Empresa\t\t\tImproductividad\n", "Delegacion", "Descripcion");
...
void ImprimirDetalle()
{
	...
	if (FmIFld(fm0, DETCLI)) {
		...
		strcpy(regimenpto, NULL_STR);
		SetKey(operac|PUESTOS, evig->cli, evig->obj, evig->ptoser, evig->puesto);
		if (GetRecord(operac|PUESTOS, THIS_KEY, IO_NOT_LOCK) != ERROR) {
			strcpy(regimenpto, SFld(operac|PUESTOS_REGIM));
		}

		RpSetFld (rp0, RREGIMP, regimenpto);
...
static fm_status after(form fm, fmfield fno, int row)
{
	...
	case EMP:
		if (FmChgFld(fm)){
		  	FmSetFld(fm0, COMENT, "[1mProcesando Permisos de Filiales por Usuario[0m");
		  	WiRefresh();
			InicListaXusr(FmIFld(fm, fno));
			InicLegajoXusr (FmIFld(fm, fno), fecierre, fm0, COMENT, TRUE, _TIPPER_INSERTA);
			FmSetFld(fm0, COMENT, "");
			WiRefresh();
		}
	    break;
	case SALIDA:
		if (*FmSFld(fm0, SALIDA) == 'A' && FmIsNull(fm0, NOMARCH))
			FmSetFld(fm0, NOMARCH, "hsvigil.txt");
		break;
	case FECHAD:
...
//			ImprimeLegajoXusr();
	...
	case VIGILH:
		switch(FmKeyCode(fm)) {
	  	...
	  	FmSetFld(fm, fno + 1, GetDescLegajo(FmIFld(fm0, EMP), FmLFld(fm, fno, row)), row);
...
void ImprimirDetArch()
{
	...
	if (!FmIFld(fm0, DETCLI))
		...
		fprintf(fp, "%d\t", evig->ptoser);
		fprintf(fp, "%s\t", GetDescPto(evig->ptoser));
		fprintf(fp, "%d\t", evig->puesto);
		fprintf(fp, "%.1D\t", evig->fecha);
		fprintf(fp, "%s\t", regimen);
		fprintf(fp, "%s\t", regimenpto);
...
void TotLeg()
{
	if (*FmSFld(fm0, R_DETALLE) == _POR_DELEG)
		return;
