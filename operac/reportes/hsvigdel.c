/********************************************************************
*
* MODULE & VERSION : @(#)hsvigdel.c	1.41
* DATE             : 06/05/24
* TIME             : 18:02:47
*
...
#include "comerc.h"
#include "billpro.h"
#include "hsvigdel.fmh"
#include "ambiente.h"

#ifdef _NOVIA_VER_1_0
	#include "hsvigdel.rph"
#else	
	#include "hsvigdel2.rph"
#endif
...
wcmd(hsvigdel, 1.41 05/24/06)
{
	...
	fm0 = OpenForm("hsvigdel", FM_EABORT);
...
{
	#ifdef _NOVIA_VER_1_0
	    rp0 = OpenReport("hsvigdel", RP_EABORT|RP_NOBEGIN);
	#else	
	    rp0 = OpenReport("hsvigdel2", RP_EABORT|RP_NOBEGIN);
	#endif
...
static fm_status after(form fm, fmfield fno, int row)
{
	...
	case SALIDA:
		if (*FmSFld(fm0, SALIDA) == 'A' && FmIsNull(fm0, NOMARCH))
			FmSetFld(fm0, NOMARCH, "hsvigdel.txt");
	break;
	case FECHAD:
		#ifdef _NOVIA_VER_2_0
