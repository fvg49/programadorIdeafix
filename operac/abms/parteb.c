...
#include "parteb.fmh"        //fm0 (parteb.fm)
#include "parteb1.fmh"       //fm1 (parteb1.fm)	Detalle de Horas
#include "parteb2.fmh"       //fm2 (parteb2.fm)	Excepciones
#include "parteb3.fmh"       //fm3 (parteb3.fm)	Ausentimo
#include "parteb4.fmh"       //fm4 (parteb4.fm)	Asignación del puesto
#include "parteb5.fmh"       //fm5 (parteb5.fm) Control de calculo de horas
...
static fm_status before(form, fmfield, int), after(form, fmfield, int),
				 							 after_parteb2(form, fmfield, int),
											 after_parteb4(form, fmfield, int),
...
/* Programa principal */
wcmd(parteb, 1.4 01/03/11)
{
	fm_cmd cmd;

	sprintf(g_prog, "%s", argv[0]);

	fm0 = OpenForm("parteb", FM_EABORT);
...
//	fprintf(stderr,"empresa= %d",g_emp);
//	ImprimeLegajoXusr();
	...
	case COND:
		...
		if (FmChgFld(fm)){
			...
			for (v_i=0; v_i < FmFldLen(fm0, MULTIPAR) && !FmIsNull(fm0, NROLEG, v_i); v_i++) {
				...
				CalculoDetalleHorasPer(TRUE, fm0, v_i, FmIFld(fm0, EMP), FmLFld(fm0, CLIE), FmIFld(fm0, OBJET), 
	                       FmDFld(fm0, FECPARTE), FmLFld(fm0, NROLEG, v_i), FmIFld(fm4, TIPPTO), FmIFld(fm4, CODINT), 
                           FmIFld(fm4, NROINT),&g_hsnor, &g_hs50, &g_hs100f, &g_hs100fe, &g_hspega, &g_guardi);
...
//		fm1 = UseSubform(fm, DETHS, 0, row);
		...
		if (FmChgFld(fm) || !renglon_ok) {
			...
			for (v_i=0; v_i < FmFldLen(fm0, MULTIPAR) && !FmIsNull(fm0, NROLEG, v_i); v_i++) {
				...
				//fprintf(stderr, "PARTE g_hsnor %d, g_hs50 %d, g_hs100f %d, g_hs100fe %d, g_hspega %d\n", g_hsnor, g_hs50, g_hs100f, g_hs100fe, g_hspega);
...
*/
	...
	case PTO:
		...
		if (*FmSFld(fm, COND, row) == 'T' || FmIsNull(fm, COND, row)) {
			if ((FmKeyCode(fm) == K_META || newvig || FmIsNull(fm4, TIPPTO)) && FmKeyCode(fm)!= K_DEL) {
				...
				DoSubform(fm, NULLFP, after_parteb4, fno, 0, row);
				if (objconot)
					return FM_REDO;
				newvig = FALSE;
			}
	...
	case EXCP :
		if (FmKeyCode(fm) == K_META)
			DoSubform(fm, NULLFP, after_parteb2, fno, 0, row);
...
static fm_status after_parteb2(form fm, fmfield fno, int row)
{
...
static fm_status after_parteb4(form fm, fmfield fno, int row)
{
...
//				FmNextFld(fm0, COND,    row);
	...
	if (ErrorSuperposicion()) 
		WiDialog(WD_OK, WD_OK, TituloMsg(TMSG_WAR, 1037), 
		"No se podran grabar los datos por existir superposición.");
	...
	if (ErrPue(FmIFld(fm0, EMP), FmLFld(fm0, NROLEG, row), FmDFld(fm0, FECPARTE), FmLFld(fm0, CLIE),
		...
		FmSFld(fm0, COND, row), cadena, row);
	}
	else {
...
int  ErrPueFm(int p_row) 
{
	int v_i=0,  v_respuesta=NULL_SHORT;
	char v_orig[10], v_dest[10];
	...
	for (v_i=0; v_i <FmFldLen(fm0, MULTIPAR) && !FmIsNull(fm0, NROLEG, v_i); v_i++) {
		...
		sprintf(v_orig, "%s", FmSFld(fm0, COND, p_row));
		sprintf(v_dest, "%s", FmSFld(fm0, COND, v_i));

		if (strcmp(v_orig, v_dest)!=0) {
			v_respuesta=v_i;
			break;
		}
					 ...
					 int p_hsnor, int p_hs50, int p_hs100fe, int p_hs100f, int p_codaus)
{
	...
	if (p_hsnor  != 0 || p_hs50   != 0 || p_hs100fe != 0 || p_hs100f != 0){
		...
		CalculoDetalleHorasPer(TRUE, fm0, p_i, p_emp, FmLFld(fm0, CLIE), FmIFld(fm0, OBJET), p_dia, p_nroleg, p_ptoser, p_puesto, p_nroint, 
		                        &g_hsnor, &g_hs50, &g_hs100f, &g_hs100fe, &g_hspega, &g_guardi);

WiMsg("row %d %f %f %f", p_i, g_hsnor, g_hs50, g_hs100fe);
